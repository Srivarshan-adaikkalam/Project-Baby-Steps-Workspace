...
*   **Mandatory Skill Bootstrapping**: At the beginning of any session, the agent MUST read the Global Skills Registry in `WORKSPACE_MAP.md`. The agent must verify if all listed skills exist in the workspace. If any skill is missing, the agent MUST automatically `git clone` or download the missing repositories into the `scratch/` space or `.agents/skills/` BEFORE executing any code.
...
*   **SEO Optimization Mandate**: When building or modifying Static HTML architectures, the agent MUST automatically implement strict on-page technical SEO. All core copy MUST be hardcoded directly into the HTML (not injected by JS). Ensure semantic hierarchy (a single `<h1>`, proper `<h2>`/`<h3>` usage), `alt` attributes on all media, and descriptive `<title>` and `<meta name="description">` tags.
